        ...
        <div className="flex justify-between h-16 items-center">
          <Link to="/" className="flex items-center gap-3 group active:scale-95 transition-transform">
            <div className="relative">
              <div className="bg-gradient-to-br from-blue-500 to-blue-600 w-11 h-11 rounded-[14px] text-white flex items-center justify-center shadow-lg shadow-blue-500/20">
                <i className="fas fa-file-pdf text-xl"></i>
              </div>
              <div className="absolute -top-1 -right-1 bg-blue-400 w-4 h-4 rounded-full border-2 border-white dark:border-slate-950 flex items-center justify-center">
            ...
            <div className="flex flex-col -space-y-1">
              <span className="text-xl font-[900] tracking-tighter text-slate-900 dark:text-white uppercase leading-none">
                PDF<span className="text-blue-600">Vio</span>
              </span>
    ...
    <div className="md:hidden fixed bottom-0 left-0 right-0 z-[60] px-4 pb-6 pointer-events-none">
      <div className="bg-white/90 dark:bg-slate-950/90 backdrop-blur-xl border border-slate-200/50 dark:border-slate-800/50 rounded-[2.5rem] shadow-[0_-10px_40px_-15px_rgba(0,0,0,0.1)] flex items-center justify-between px-6 py-3 pointer-events-auto">
        {navItems.map((item) => (
          ...
          >
            {item.isCenter ? (
              <div className="w-14 h-14 bg-gradient-to-tr from-blue-500 to-blue-600 rounded-full flex items-center justify-center text-white text-xl shadow-xl shadow-blue-500/40 ring-4 ring-white dark:ring-slate-950">
                <i className={`fas ${item.icon}`}></i>
              </div>
            ) : (
              <>
                <i className={`fas ${item.icon} text-sm ${path === item.path ? 'text-blue-600' : 'text-slate-400 dark:text-slate-500'}`}></i>
                <span className={`text-[8px] font-black uppercase tracking-widest ${path === item.path ? 'text-blue-600' : 'text-slate-400 dark:text-slate-500'}`}>
                  {item.label}
                </span>
              </>
            )}
      ...
      <footer className={`bg-white dark:bg-slate-950 border-t border-slate-100 dark:border-slate-900 py-12 px-6 hidden md:${isScannerActive ? 'hidden' : 'block'}`}>
        <div className="max-w-5xl mx-auto flex flex-col items-center gap-6">
          <div className="flex gap-6 text-[10px] font-black text-slate-400 dark:text-slate-600 uppercase tracking-widest">
            <Link to="/legal" className="hover:text-blue-600 transition-colors">Privacy Policy</Link>
            <Link to="/legal" className="hover:text-blue-600 transition-colors">Terms of Service</Link>
            <Link to="/blog" className="hover:text-blue-600 transition-colors">Blog</Link>
          </div>
          <div className="text-[11px] font-black tracking-widest text-slate-300 dark:text-slate-800 uppercase">
            © 2026 PDFVio PRO SUITE • BUILT FOR PRIVACY
          </div>
        </div>
      </footer>
