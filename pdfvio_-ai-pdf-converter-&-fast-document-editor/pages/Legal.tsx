    ...
    <div className="max-w-4xl mx-auto px-6 py-16">
      <div className="flex gap-4 mb-12 bg-slate-100 dark:bg-slate-900 p-2 rounded-2xl w-fit mx-auto">
        <button 
          ...
          className={`px-8 py-3 rounded-xl text-xs font-black uppercase tracking-widest transition-all ${tab === 'privacy' ? 'bg-white dark:bg-slate-800 text-blue-600 shadow-sm' : 'text-slate-500'}`}
        >
          Privacy Policy
        </button>
        <button 
          onClick={() => setTab('terms')}
          className={`px-8 py-3 rounded-xl text-xs font-black uppercase tracking-widest transition-all ${tab === 'terms' ? 'bg-white dark:bg-slate-800 text-blue-600 shadow-sm' : 'text-slate-500'}`}
        >
          Terms of Service
        </button>
      </div>
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 p-8 md:p-12 rounded-[3rem] shadow-sm prose prose-slate dark:prose-invert max-w-none">
        {tab === 'privacy' ? (
          <div>
            ...
            <p>PDFVio is built on a "Privacy-First" architecture. All PDF manipulations, conversions, and scanning processes happen exclusively within your device's browser memory (Client-Side). We do not upload your documents to any server.</p>
            ...
            <h2 className="text-xl font-bold mt-8 mb-4">4. Cookies and Advertising</h2>
            <p>We use cookies to improve your experience and show relevant advertisements. Google, as a third-party vendor, uses cookies to serve ads on our site. Google's use of advertising cookies enables it and its partners to serve ads to our users based on their visit to our sites and/or other sites on the Internet.</p>
            <p>Users may opt out of personalized advertising by visiting <a href="https://www.google.com/settings/ads" target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline">Google Ad Settings</a>.</p>
          </div>
        ) : (
          <div>
            ...
            <p>By using PDFVio, you agree to these terms. This suite is provided "as-is" for document management purposes.</p>
            <h2 className="text-xl font-bold mt-8 mb-4">2. Limitation of Liability</h2>
            <p>While we strive for 100% accuracy, PDFVio is not responsible for any data loss or document corruption that may occur during browser-based processing. Always keep backups of your original files.</p>
